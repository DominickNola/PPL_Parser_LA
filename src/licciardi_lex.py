...
def lex(input):
    ...
    # reads punctuation words
    if charClass == CharClass.PUNCTUATION:
        input, lexeme = addChar(input, lexeme)
        if lexeme in lookupWord:
            return input, lexeme, lookupPunctuation[lexeme]
...
# given an input (source program), a grammar, actions, and gotos, returns the corresponding parse tree or raise
# an exception if syntax errors were found
